...
// import "../interfaces/ISwapNetworkProxyInterface.sol";
import "../interfaces/ISwapInterface.sol";
...
///  - If the address(0) is used as receiver, this contract will trade in Swap to tokenToBurn (LEND)
...
contract TokenDistributor is VersionedInitializable {
    ...
    ISwapInterface public SwapProxy;

    /// @notice The address of the token to burn (LEND token)
    address public tokenToBurn;

    /// @notice Address to send tokens to "burn".
    /// Because of limitations on OZ TRC20, on dev it's needed to use the 0x00000...1 address instead of address(0)
    ...
    function initialize(
        ...
        ITRC20[] memory _tokens
    ) public initializer {
        recipientBurn = _recipientBurn;
        tokenToBurn = _tokenToBurn;
        SwapProxy = ISwapInterface(_SwapProxy);
    ...
    /// @param _tokens list of TRC20 tokens to distribute
    function distribute(ITRC20[] memory _tokens) public {
    ...
    /// @param _tokens List of ITRC20 to approve
    function approveSwap(ITRC20[] memory _tokens) public {
        for (uint256 i = 0; i < _tokens.length; i++) {
    ...
    function internalBurn(uint256 _amount) internal {
        require(ITRC20(tokenToBurn).transfer(recipientBurn, _amount), "INTERNAL_BURN. Reverted transfer to recipientBurn address");
        emit Burn(_amount);
    }
