...
contract LendingPoolCore is VersionedInitializable {
    ...
    * @param _balanceIncrease the accrued interest on the borrowed amount
    * @param _liquidatorReceivesWToken true if the liquidator will receive wTokens, false otherwise
    **/
    function updateStateOnLiquidation(
        ...
        uint256 _balanceIncrease,
        bool _liquidatorReceivesWToken
    ) external onlyLendingPool {
    ...
    **/
    function() external payable {
        ...
        require(msg.sender.isContract(), "Only contracts can send trx to the Lending pool core");
    ...
    **/
        ...
        onlyLendingPool
    {
        if (_reserve != TrxAddressLib.trxAddress()) {
            TRC20(_reserve).transfer(_user, _amount);
        } else {
    ...
    **/
    ...
    function transferToFeeCollectionAddress(
        ...
        if (_token != TrxAddressLib.trxAddress()) {
            require(
                msg.value == 0,
                "User is sending TRX along with the TRC20 transfer. Check the value attribute of the transaction"
            );
            TRC20(_token).transferFrom(_user, feeAddress, _amount);
        } else {
    ...
    **/
    function liquidateFee(
        ...
        if (_token != TrxAddressLib.trxAddress()) {
            TRC20(_token).transfer(feeAddress, _amount);
        } else {
            //solium-disable-next-line
            (bool result, ) = feeAddress.call.value(_amount).gas(50000)("");
            require(result, "Transfer of TRX failed");
    ...
    **/
    ...
    {
        if (_reserve != TrxAddressLib.trxAddress()) {
            require(msg.value == 0, "User is sending TRX along with the TRC20 transfer.");
    ...
    **/
    ...
    /**
    * @dev gets the underlying asset balance of a user based on the corresponding wToken balance.
    * @param _reserve the reserve address
    ...
    **/
    ...
    function getUserUnderlyingAssetBalance(address _reserve, address _user)
        ...
        returns (uint256)
    {
        WToken wToken = WToken(reserves[_reserve].wTokenAddress);
        return wToken.balanceOf(_user);
    ...
    **/
    ...
    /**
    * @dev gets the wToken contract address for the reserve
    * @param _reserve the reserve address
    * @return the address of the wToken contract
    **/

    function getReserveWTokenAddress(address _reserve) public view returns (address) {
        CoreLibrary.ReserveData storage reserve = reserves[_reserve];
        return reserve.wTokenAddress;
    }
    ...
    **/
    function getReserveAvailableLiquidity(address _reserve) public view returns (uint256) {
        ...
        if (_reserve == TrxAddressLib.trxAddress()) {
            ...
            balance = ITRC20(_reserve).balanceOf(address(this));
        }
        return balance;
    }
    ...
    * @param _reserve the address of the reserve
    * @param _wTokenAddress the address of the overlying wToken contract
    ...
    **/
    function initReserve(
        ...
        reserves[_reserve].init(_wTokenAddress, _decimals, _interestRateStrategyAddress);
    ...
    **/
    function removeLastAddedReserve(address _reserveToRemove)
     external onlyLendingPoolConfigurator {
        ...
        //as we can't check if totalLiquidity is 0 (since the reserve added might not be an TRC20) we at least check that there is nothing borrowed
        ...
        reserves[lastReserve].wTokenAddress = address(0);
